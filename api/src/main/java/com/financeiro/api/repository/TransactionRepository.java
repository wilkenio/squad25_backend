...
import com.financeiro.api.domain.Category; // Removido se não usado (findByAccountInAndCategoryIn usa)
...
public interface TransactionRepository extends JpaRepository<Transaction, UUID> {
    ...
    List<Transaction> findByAccountAndStatusAndReleaseDateBetween(
        Account account,
        Status status,
    ...
    List<Transaction> findByAccountAndStatusAndStateAndReleaseDateLessThanEqual(
        Account account,
        Status status,
        TransactionState state,
        LocalDateTime endDate
